...
def setup_ranger_nifi(upgrade_type=None):
    import params
    if not os.path.exists(format('{stack_root}/current/nifi-server/ext/ranger/scripts')):
      Directory(format('{stack_root}/current/nifi-server/ext/ranger/scripts'),
    ...
    if params.has_ranger_admin and params.enable_ranger_nifi:
        File(format('{stack_root}/current/nifi-server/ext/ranger/scripts/ranger_credential_helper.py'),
             owner=params.nifi_user,
             group=params.nifi_group,
             mode=0750,
             )
        ...
        cred_setup_prefix_path = (format('{stack_root}/current/nifi-server/ext/ranger/scripts/ranger_credential_helper.py'), '-l', cred_lib_prefix_path)
